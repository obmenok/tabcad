...
ui_Rc = widgets.FloatText(value=8.8002, description='Cup Radius:', disabled=True, style=style, layout=layout)
...
# Новые параметры двойного радиуса
...
def get_compound_profile(x_arr, R1, R2, Dc, span):
    """
    Математика пересечения двух окружностей для построения сложного профиля чаши.
    R1 - центральный (Major Major) радиус
    R2 - краевой (Major Minor) радиус
    """
    Z1 = Dc - R1
...
def compute_width_from_depth(depth, angle, ri, dc, R1, R2, span):
    ...
    z_cup = get_compound_profile(x_test, R1, R2, dc, span)
...
def compute_depth_from_width(target_width, angle, ri, dc, R1, R2, span):
    if target_width <= 0: return 0.0
    d_min, d_max = 0.0, dc * 2.0
    for _ in range(35):
        d_mid = (d_min + d_max) / 2
        w_mid = compute_width_from_depth(d_mid, angle, ri, dc, R1, R2, span)
...
def apply_suggested(b):
    ...
    try:
        ...
        ui_b_width.value = compute_width_from_depth(suggested_depth, 90.0, 0.061, ui_Dc.value, ui_R_maj_maj.value, ui_R_maj_min.value, r_c)
    finally:
        updating = False
        draw_views()
...
def on_value_change(change):
    ...
    try:
        ...
        # Синхронизация риски с учетом сложного профиля Compound Cup
        if sender == ui_b_width:
            calc_depth = compute_depth_from_width(ui_b_width.value, ui_b_angle.value, ui_b_Ri.value, ui_Dc.value, ui_R_maj_maj.value, ui_R_maj_min.value, r_c)
            if calc_depth > max_b_depth:
                ui_b_depth.value = max_b_depth
                ui_b_width.value = compute_width_from_depth(max_b_depth, ui_b_angle.value, ui_b_Ri.value, ui_Dc.value, ui_R_maj_maj.value, ui_R_maj_min.value, r_c)
            else:
                ui_b_depth.value = calc_depth
        elif sender in (ui_b_depth, ui_b_angle, ui_b_Ri, ui_b_type, ui_Dc, ui_R_maj_maj, ui_R_maj_min):
            ui_b_width.value = compute_width_from_depth(ui_b_depth.value, ui_b_angle.value, ui_b_Ri.value, ui_Dc.value, ui_R_maj_maj.value, ui_R_maj_min.value, r_c)

        draw_views()
    finally:
        updating = False

for widget in (ui_D, ui_Hb, ui_Tt, ui_Dc, ui_R_maj_maj, ui_R_maj_min, ui_Land, ui_b_type, ui_b_width, ui_b_depth, ui_b_angle, ui_b_Ri):
...
def draw_views():
    with out:
        ...
        Z = np.zeros_like(X)
        Z[mask_cup] = get_compound_profile(rho[mask_cup], R_maj_maj, R_maj_min, Dc, r_c)
        ...
        if b_type != 'None' and b_depth > 0:
            ...
            m_in = np.abs(X) <= r_c
            Z_centerline[m_in] = get_compound_profile(X[m_in], R_maj_maj, R_maj_min, Dc, r_c)
        ...
        # Численное интегрирование для сложного профиля
        ...
        z_up_surf = yc_up + get_compound_profile(y_cup, R_maj_maj, R_maj_min, Dc, r_c)
        ...
        z_down_surf = yc_up + get_compound_profile(y_cup, R_maj_maj, R_maj_min, Dc, r_c)
        ...
        z_up_front = xc_up + get_compound_profile(x_cup, R_maj_maj, R_maj_min, Dc, r_c)
        ...
        if b_type != 'None' and b_depth > 0:
            Z_centerline_front = get_compound_profile(x_cup, R_maj_maj, R_maj_min, Dc, r_c)
        ...
        # Выноски радиусов
        # 1. Minor Radius (на край купола слева)
        x_min_pt = r_c * 0.8
        z_min_pt = get_compound_profile(np.array([x_min_pt]), R_maj_maj, R_maj_min, Dc, r_c)[0]
        targ_front_min = (cx_front - x_min_pt, cy_front + Hb/2 + z_min_pt)
        # Сдвинут правее и немного выше
        txt_front_min = (cx_front - x_min_pt - D/6, cy_front + Hb/2 + z_min_pt + 5)
        draw_pointer(ax, targ_front_min, txt_front_min, f"{R_maj_min:g}\nMinor Radius")

        # 2. Major Radius (ближе к центру справа)
        x_maj_pt = r_c * 0.2
        z_maj_pt = get_compound_profile(np.array([x_maj_pt]), R_maj_maj, R_maj_min, Dc, r_c)[0]
        targ_front_maj = (cx_front + x_maj_pt, cy_front + Hb/2 + z_maj_pt)
        # Почти вертикально (минимальное смещение по X, большое по Y)
        txt_front_maj = (cx_front + x_maj_pt + 0.5, cy_front + Hb/2 + z_maj_pt + 5)
        draw_pointer(ax, targ_front_maj, txt_front_maj, f"{R_maj_maj:g}\nMajor Radius")
...
col2 = widgets.VBox([ui_R_maj_maj, ui_R_maj_min])
