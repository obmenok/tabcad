...
# Входные параметры (Круглая таблетка FFBE)
...
# =====================================================================
def get_ffbe_round_profile(r_arr, span, Dc, R_blend, angle_deg):
    """
    Математика профиля чаши Flat Face Bevel Edge для радиальной симметрии.
    """
    z_prof = np.zeros_like(r_arr)
    ...
    r_abs = np.abs(r_arr)
    mask_flat = r_abs <= max(0, r_flat)
    
    Rt = r_flat + R_blend * sin_a if r_flat > 0 else span - Dc / tan_a
    mask_blend = (r_abs > max(0, r_flat)) & (r_abs <= Rt)
    mask_bevel = (r_abs > Rt) & (r_abs <= span)
    
    if np.any(mask_flat):
        z_prof[mask_flat] = Dc
    if np.any(mask_blend):
        z_prof[mask_blend] = (Dc - R_blend) + np.sqrt(np.maximum(0, R_blend**2 - (r_abs[mask_blend] - r_flat)**2))
    if np.any(mask_bevel):
        z_prof[mask_bevel] = (span - r_abs[mask_bevel]) * tan_a
...
def compute_width_from_depth(depth, angle, ri, dc, R_blend, angle_deg, span):
    ...
    z_cup = get_ffbe_round_profile(x_test, span, dc, R_blend, angle_deg)
...
def on_value_change(change):
    ...
    try:
        ...
        # === ИНТЕЛЛЕКТУАЛЬНЫЙ ПРЕДОХРАНИТЕЛЬ ДЛЯ ФАСКИ И СКРУГЛЕНИЯ ===
        ...
        if sender in (ui_D, ui_Dc, ui_Land, ui_Bev_A, ui_Blend_R):
...
for widget in (ui_D, ui_Hb, ui_Tt, ui_Dc, ui_Blend_R, ui_Bev_A, ui_Land, ui_b_type, ui_b_width, ui_b_depth, ui_b_angle, ui_b_Ri):
...
def draw_views():
    with out:
        ...
        # 3D Генерация
        Z = np.zeros_like(X)
        Z[mask_cup] = get_ffbe_round_profile(rho[mask_cup], r_c, Dc, R_blend, Bev_A)
        ...
        if b_type != 'None' and b_depth > 0:
            ...
            m_in = np.abs(X) <= r_c
            Z_centerline[m_in] = get_ffbe_round_profile(X[m_in], r_c, Dc, R_blend, Bev_A)
        ...
        if r_flat > 0.05:
            x_flat, y_flat = get_circle_contour(r_flat)
        ...
        z_up_surf = yc_up + get_ffbe_round_profile(y_cup, r_c, Dc, R_blend, Bev_A)
        ...
        z_down_surf = yc_up + get_ffbe_round_profile(y_cup, r_c, Dc, R_blend, Bev_A)
        ...
        z_up_front = xc_up + get_ffbe_round_profile(x_cup, r_c, Dc, R_blend, Bev_A)
        ...
        if b_type != 'None' and b_depth > 0:
            Z_centerline_front = get_ffbe_round_profile(x_cup, r_c, Dc, R_blend, Bev_A)
        ...
        if r_flat > 0:
            draw_ext_outside(ax, cx_front - r_flat, cy_front - Hb/2 - Dc, cx_front + r_flat, cy_front - Hb/2 - Dc, 0, -3, f"{2*r_flat:.3f}\nRef. Flat")

        # Выноска Blend Radius (указывает на скругление слева вверху)
        w_target_val = - (r_c - D_inset + (R_blend * sin_a)/2) 
        z_min_pt = get_ffbe_round_profile(np.array([w_target_val]), r_c, Dc, R_blend, Bev_A)[0]
        ...
        if Bev_A > 0 and Bev_A < 90:
            ...
            # Наклонная линия фаски (влево и вниз)
            dx = ext_len * cos_a
            dy = -ext_len * sin_a
            ax.plot([pt_x, pt_x + dx], [pt_y, pt_y + dy], 'k-', lw=DIM_LINE_WIDTH)

            # Дуга (от угла -alpha_rad до 0)
...
col2 = widgets.VBox([ui_Bev_A, ui_Blend_R])
