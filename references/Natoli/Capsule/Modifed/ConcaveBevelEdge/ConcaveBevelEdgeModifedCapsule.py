...
# Входные параметры (CBE Modified Capsule)
...
ui_Re = widgets.FloatText(value=4.26, step=0.01, description='End Radius:', style=style, layout=layout)
ui_Rs = widgets.FloatText(value=39.594, step=0.01, description='Side Radius:', style=style, layout=layout)

ui_Dc = widgets.FloatText(value=1.5, step=0.01, description='Cup Depth:', style=style, layout=layout)
ui_Rc = widgets.FloatText(value=8.225, description='Cup Radius Minor:', disabled=True, style=style, layout=layout)
...
ui_Tt = widgets.FloatText(value=5.54, step=0.01, description='Tablet Thick:', style=style, layout=layout)
...
# =====================================================================
# 3. МАТЕМАТИКА 3D СВОДА CBE И РИСКИ
# =====================================================================
def get_capsule_cbe_profile(rho_arr, rc, Dc, Db, angle_deg):
    """Возвращает 3D профиль фаска+сфера по заданному радиусу rho от хребта капсулы"""
    ...
    if np.any(mask_cup):
        if hc > 0:
            Rc = (r_in**2 + hc**2) / (2 * hc)
            z_prof[mask_cup] = Db + np.sqrt(np.maximum(0, Rc**2 - rho_arr[mask_cup]**2)) - (Rc - hc)
        else:
            z_prof[mask_cup] = Db
            
    return np.maximum(0, z_prof)

def compute_width_from_depth(depth, angle, ri, dc, db, angle_deg, rc):
    ...
    z_cup = get_capsule_cbe_profile(x_test, rc, dc, db, angle_deg)
...
def compute_depth_from_width(target_width, angle, ri, dc, db, angle_deg, rc):
    if target_width <= 0: return 0.0
    d_min, d_max = 0.0, dc * 2.0
    for _ in range(35):
        d_mid = (d_min + d_max) / 2
        w_mid = compute_width_from_depth(d_mid, angle, ri, dc, db, angle_deg, rc)
...
def apply_suggested(b):
    ...
    try:
        ...
        ui_b_width.value = compute_width_from_depth(suggested_depth, 90.0, 0.061, ui_Dc.value, ui_Bev_D.value, ui_Bev_A.value, r_c)
    finally:
        updating = False
        draw_views()
...
# =====================================================================
# 4. ЛОГИКА ВАЛИДАЦИИ И СИНХРОНИЗАЦИИ
# =====================================================================
updating = False
...
def on_value_change(change):
    ...
    try:
        ...
        ui_Bev_D.value = max(0.0, ui_Bev_D.value)
        ...
        # Строгая геометрия сопряжения дуг Овала
        ...
        # Предохранитель для Фаски
        alpha = np.radians(ui_Bev_A.value)
        tan_a = np.tan(alpha)
        
        if ui_Bev_D.value > Dc:
            ui_Bev_D.value = round(Dc, 4)
            
        max_db = r_c * tan_a
        if ui_Bev_D.value > max_db - 0.001:
            ui_Bev_D.value = round(max_db - 0.001, 4)
        
        Db = ui_Bev_D.value

        # Синхронизация эквивалентного сферического радиуса CBE
        if sender in (ui_W, ui_L, ui_Dc, ui_Land, ui_Bev_D, ui_Bev_A, ui_Re, ui_Rs):
            hc = Dc - Db
            w_b = Db / tan_a if tan_a > 0 else 0
            r_in = r_c - w_b
            if hc > 0:
                ui_Rc.value = round((r_in**2 + hc**2) / (2 * hc), 4)
            else:
                ui_Rc.value = 0.0
        ...
        if sender == ui_b_width:
            calc_depth = compute_depth_from_width(ui_b_width.value, ui_b_angle.value, ui_b_Ri.value, ui_Dc.value, Db, ui_Bev_A.value, r_c)
            if calc_depth > max_b_depth:
                ui_b_depth.value = max_b_depth
                ui_b_width.value = compute_width_from_depth(max_b_depth, ui_b_angle.value, ui_b_Ri.value, ui_Dc.value, Db, ui_Bev_A.value, r_c)
            else:
                ui_b_depth.value = calc_depth
        elif sender in (ui_b_depth, ui_b_angle, ui_b_Ri, ui_b_type, ui_Dc, ui_W, ui_L, ui_Land, ui_Bev_D, ui_Bev_A):
            ui_b_width.value = compute_width_from_depth(ui_b_depth.value, ui_b_angle.value, ui_b_Ri.value, ui_Dc.value, Db, ui_Bev_A.value, r_c)

        draw_views()
    finally:
        updating = False

for widget in (ui_W, ui_L, ui_Re, ui_Rs, ui_Hb, ui_Tt, ui_Dc, ui_Bev_D, ui_Bev_A, ui_Land, ui_b_type, ui_b_width, ui_b_depth, ui_b_angle, ui_b_Ri):
...
def draw_views():
    with out:
        ...
        Dc, Rc_disp = ui_Dc.value, ui_Rc.value
        Db, Bev_A = ui_Bev_D.value, ui_Bev_A.value
        ...
        w_b = Db / np.tan(alpha_rad) if alpha_rad > 0 else 0
        r_c = W/2 - Land
        r_in = max(0, r_c - w_b)

        # Вычисление площади для Овала
        xe, ys = L / 2 - Re, W / 2 - Rs
        gamma = np.arctan2(abs(ys), xe)
        ...
        # 3D Контур: Вытянутый капсульный свод, как в строгой капсуле
        ...
        Z = np.zeros_like(X)
        Z[mask_cup] = get_capsule_cbe_profile(rho_3d[mask_cup], r_c, Dc, Db, Bev_A)
        ...
        if b_type != 'None' and b_depth > 0:
            ...
            m_in = np.abs(X) <= r_c 
            Z_centerline[m_in] = get_capsule_cbe_profile(np.abs(Y[m_in]), r_c, Dc, Db, Bev_A)
        ...
        # Линия перехода фаски в сферу
        if w_b > 0 and Re - Land - w_b > 0.1:
            try:
                x_in2, y_in2 = get_oval_contour(L - 2*Land - 2*w_b, W - 2*Land - 2*w_b, Re - Land - w_b, Rs - Land - w_b)
                ax.plot(y_in2 + cx_top, x_in2 + cy_top, 'k-', linewidth=0.6)
            except: pass
        ...
        z_up_maj_surf = get_capsule_cbe_profile(rho_maj, r_c, Dc, Db, Bev_A) + yc_up_maj
        ...
        z_down_maj_surf = get_capsule_cbe_profile(rho_maj, r_c, Dc, Db, Bev_A) + yc_up_maj
        ...
        if Db > 0: draw_ext_outside(ax, cx_side + Hb/2, cy_side + L/2, cx_side + Hb/2 + Db, cy_side + L/2, 0, 2, f"{Db:g}\nBevel Depth")
        ...
        z_up_min = yc_up_min + get_capsule_cbe_profile(rho_min, r_c, Dc, Db, Bev_A)
        ...
        if b_type != 'None' and b_depth > 0:
            Z_centerline_front = get_capsule_cbe_profile(rho_min, r_c, Dc, Db, Bev_A)
        ...
        if Rc_disp > 0:
            draw_pointer(ax, (cx_front, cy_front + Hb/2 + Dc), (cx_front - W/4, cy_front + Hb/2 + Dc + 4), f"{Rc_disp:g}\nCup Radius\nMinor")

        if Bev_A > 0 and Bev_A < 90 and Db > 0:
            pt_x = cx_front + r_c
            pt_y = cy_front + Hb/2
            ext_len = W / 4.0
            ...
            ax.text(pt_x + (arc_r + text_offset) * np.cos(mid_ang), pt_y + (arc_r + text_offset) * np.sin(mid_ang), 
                    f"{Bev_A:g}°", color=C_TEXT, ha='center', va='center', fontsize=9, bbox=dict(facecolor='#f4f4f9', edgecolor='none', pad=0.5))
...
col4 = widgets.VBox([ui_Bev_D, ui_Bev_A])
