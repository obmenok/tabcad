...
# Входные параметры (Размеры таблетки - по умолчанию Modified Oval)
...
ui_Dc = widgets.FloatText(value=1.2, step=0.01, description='Cup Depth:', style=style, layout=layout)
ui_Rc_min = widgets.FloatText(value=9.1127, step=0.01, description='Cup Radius Minor:', style=style, layout=layout)
ui_R_maj_maj = widgets.FloatText(value=88.9, step=0.1, description='Major Major Rad:', style=style, layout=layout)
ui_R_maj_min = widgets.FloatText(value=12.7, step=0.1, description='Major Minor Rad:', style=style, layout=layout)
...
ui_Tt = widgets.FloatText(value=5.04, step=0.01, description='Tablet Thick:', style=style, layout=layout)

# Входные параметры (Риска)
ui_b_type = widgets.Dropdown(options=['None', 'Standard', 'Cut Through', 'Decreasing'], value='Cut Through', description='Bisect Type:', style=style, layout=layout)
...
# Выходные параметры (Площади и Объемы)
...
# =====================================================================
# 3. МАТЕМАТИКА СОПРЯЖЕНИЯ И РИСКИ
# =====================================================================
def get_modified_major_profile(x_arr, R1, R2, Dc, lc):
    """
    Рассчитывает профиль купола с двумя сопряженными радиусами (Major Major и Major Minor).
    Находит точку идеального касания двух окружностей алгебраически.
    """
    Z1 = Dc - R1
    
    if lc <= 0 or R1 <= 0 or R2 <= 0: return np.zeros_like(x_arr)
    
    # Решение системы уравнений пересечения окружностей
    K = ((R1 - R2)**2 - R2**2 + lc**2 - Z1**2) / 2.0
    M = (K / lc) - lc
    N = Z1 / lc
    ...
    if discriminant < 0:
        return np.maximum(0, Dc * (1 - (x_arr/lc)**2))
        
    zc = (-B + np.sqrt(discriminant)) / (2 * A)
    xc = (K + Z1 * zc) / lc
    
    # X-координата точки касания
    Xt = xc * R1 / (R1 - R2) if R1 != R2 else lc
    
    x_abs = np.abs(x_arr)
    z_prof = np.zeros_like(x_arr)
    
    mask_center = x_abs <= Xt
    mask_edge = (x_abs > Xt) & (x_abs <= lc)
...
def compute_width_from_depth(depth, angle, ri, dc, R1, R2, lc):
    if depth <= 0: return 0.0
    x_test = np.linspace(0, lc, 5000)
    z_bottom = dc - depth
    ...
    z_cup = get_modified_major_profile(x_test, R1, R2, dc, lc)
...
def compute_depth_from_width(target_width, angle, ri, dc, R1, R2, lc):
    if target_width <= 0: return 0.0
    d_min, d_max = 0.0, dc * 2.0
    for _ in range(35):
        d_mid = (d_min + d_max) / 2
        w_mid = compute_width_from_depth(d_mid, angle, ri, dc, R1, R2, lc)
...
def apply_suggested(b):
    ...
    try:
        ...
        ui_b_width.value = compute_width_from_depth(suggested_depth, 90.0, 0.061, ui_Dc.value, ui_R_maj_maj.value, ui_R_maj_min.value, l_c)
    finally:
        updating = False
        draw_views()
...
def on_value_change(change):
    ...
    try:
        ...
        ui_R_maj_maj.value = max(0.01, ui_R_maj_maj.value)
        ui_R_maj_min.value = max(0.01, ui_R_maj_min.value)
        ...
        if sender == ui_b_width:
            ui_b_depth.value = compute_depth_from_width(ui_b_width.value, ui_b_angle.value, ui_b_Ri.value, ui_Dc.value, ui_R_maj_maj.value, ui_R_maj_min.value, l_c)
        elif sender in (ui_b_depth, ui_b_angle, ui_b_Ri, ui_b_type, ui_Dc, ui_R_maj_maj, ui_R_maj_min):
            ui_b_width.value = compute_width_from_depth(ui_b_depth.value, ui_b_angle.value, ui_b_Ri.value, ui_Dc.value, ui_R_maj_maj.value, ui_R_maj_min.value, l_c)

        draw_views()
    finally:
        updating = False

for widget in (ui_W, ui_L, ui_Hb, ui_Tt, ui_Re, ui_Rs, ui_Dc, ui_Rc_min, ui_R_maj_maj, ui_R_maj_min, ui_Land, ui_b_type, ui_b_width, ui_b_depth, ui_b_angle, ui_b_Ri):
...
def draw_views():
    with out:
        ...
        Dc, Rc_min = ui_Dc.value, ui_Rc_min.value
        R_maj_maj, R_maj_min = ui_R_maj_maj.value, ui_R_maj_min.value
        ...
        Z_L = get_modified_major_profile(X, R_maj_maj, R_maj_min, Dc, l_c)
        Z_W = np.maximum(0, np.sqrt(np.maximum(0, Rc_min**2 - Y**2)) - (Rc_min - Dc))
        ...
        # --- 2. SIDE VIEW (Разрез профиля L с двумя радиусами) ---
        yc_up_maj = Hb/2
        x_maj_cup = np.linspace(-l_c, l_c, 400)
        z_up_maj_surf = yc_up_maj + get_modified_major_profile(x_maj_cup, R_maj_maj, R_maj_min, Dc, l_c)
        ...
        z_down_maj_surf = yc_up_maj + get_modified_major_profile(x_maj_cup, R_maj_maj, R_maj_min, Dc, l_c)
        ...
        # Стрелка для Major Major Radius (перенесена в верхнюю половину купола)
        x_maj_pt = l_c * 0.3 
        z_maj_pt = get_modified_major_profile(np.array([x_maj_pt]), R_maj_maj, R_maj_min, Dc, l_c)[0]
        ...
        # Стрелка для Major Minor Radius
        x_min_pt = l_c * 0.85
        z_min_pt = get_modified_major_profile(np.array([x_min_pt]), R_maj_maj, R_maj_min, Dc, l_c)[0]
...
col3 = widgets.VBox([ui_Dc, ui_Rc_min, ui_R_maj_maj, ui_R_maj_min])
