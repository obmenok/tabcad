...
# Входные параметры (Flat Face Radius Edge)
...
ui_Hb = widgets.FloatText(value=1.50, step=0.01, description='Belly Band:', style=style, layout=layout)
ui_Tt = widgets.FloatText(value=4.34, step=0.01, description='Tablet Thick:', style=style, layout=layout)
...
# =====================================================================
def get_ffre_profile(x_arr, span, Dc, R_edge):
...
def compute_width_from_depth(depth, angle, ri, dc, r_edge, span):
    ...
    z_cup = get_ffre_profile(x_test, span, dc, r_edge)
...
def apply_suggested(b):
    ...
    try:
        ...
        ui_b_width.value = compute_width_from_depth(suggested_depth, 90.0, 0.061, ui_Dc.value, ui_R_edge.value, w_c)
    finally:
        updating = False
        draw_views()
...
# =====================================================================
# 4. ЛОГИКА ВАЛИДАЦИИ (С УМНЫМ ПРЕДОХРАНИТЕЛЕМ ДЛЯ RADIUS)
# =====================================================================
updating = False
...
def on_value_change(change):
    ...
    try:
        ...
        # === ИНТЕЛЛЕКТУАЛЬНЫЙ ПРЕДОХРАНИТЕЛЬ ДЛЯ РАДИУСА СКРУГЛЕНИЯ ===
        current_r = max(Dc + 0.01, ui_R_edge.value)
        max_r_re = (re_c**2 + Dc**2) / (2 * Dc) if Dc > 0 else current_r
        max_r_w = (w_c**2 + Dc**2) / (2 * Dc) if Dc > 0 else current_r
        max_valid_radius = min(max_r_w, max_r_re)
        
        if current_r > max_valid_radius:
            ui_R_edge.value = round(max_valid_radius, 4)
        else:
        ...
        if sender == ui_b_width:
            ui_b_depth.value = compute_depth_from_width(ui_b_width.value, ui_b_angle.value, ui_b_Ri.value, ui_Dc.value, ui_R_edge.value, w_c)
        elif sender in (ui_b_depth, ui_b_angle, ui_b_Ri, ui_b_type, ui_Dc, ui_R_edge):
            ui_b_width.value = compute_width_from_depth(ui_b_depth.value, ui_b_angle.value, ui_b_Ri.value, ui_Dc.value, ui_R_edge.value, w_c)

        draw_views()
    finally:
        updating = False

for widget in (ui_W, ui_L, ui_Hb, ui_Tt, ui_Re, ui_Rs, ui_Dc, ui_R_edge, ui_Land, ui_b_type, ui_b_width, ui_b_depth, ui_b_angle, ui_b_Ri):
...
def draw_views():
    with out:
        ...
        Dc, R_edge = ui_Dc.value, ui_R_edge.value
        ...
        Z_L = get_ffre_profile(X, l_c, Dc, R_edge)
        Z_W = get_ffre_profile(Y, w_c, Dc, R_edge)
        Z = np.zeros_like(X)
        Dc_safe = max(1e-5, Dc)
        ...
        if b_type != 'None' and b_depth > 0:
            ...
            Z_centerline = get_ffre_profile(Y, w_c, Dc, R_edge)
        ...
        l_flat = max(0, l_c - dx_curve)
        w_flat = max(0, w_c - dx_curve)
        
        # Отрисовка контура плоской части (сплошной линией)
        if l_flat > 0.05 and w_flat > 0.05:
            L_flat = L - 2*Land - 2*dx_curve
            W_flat = W - 2*Land - 2*dx_curve
        ...
        z_up_maj_surf = yc_up_maj + get_ffre_profile(x_maj_cup, l_c, Dc, R_edge)
        ...
        z_down_maj_surf = yc_up_maj + get_ffre_profile(x_maj_cup, l_c, Dc, R_edge)
        ...
        z_up_min = yc_up_min + get_ffre_profile(x_min_cup, w_c, Dc, R_edge)
        ...
        if b_type != 'None' and b_depth > 0:
            Z_centerline_front = get_ffre_profile(x_min_cup, w_c, Dc, R_edge)
        ...
        # ИСПРАВЛЕНИЕ: Отступ dy изменен на -3 (чтобы было немного повыше, ближе к таблетке)
        if w_flat > 0:
            draw_ext_outside(ax, cx_front - w_flat, cy_front - Hb/2 - Dc, cx_front + w_flat, cy_front - Hb/2 - Dc, 0, -3, f"{2*w_flat:.3f}\nRef. Flat")

        w_target_val = - (w_c - dx_curve * 0.5) 
        z_min_pt = get_ffre_profile(np.array([w_target_val]), w_c, Dc, R_edge)[0]
...
col3 = widgets.VBox([ui_Dc, ui_Rc_min, ui_R_edge])
