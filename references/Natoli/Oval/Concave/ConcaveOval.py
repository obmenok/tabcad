...
ui_Dc = widgets.FloatText(value=1.18, step=0.01, description='Cup Depth:', style=style, layout=layout)
ui_Rc_min = widgets.FloatText(value=9.2469, step=0.01, description='Cup Rad Minor:', style=style, layout=layout)
ui_Rc_maj = widgets.FloatText(value=35.4480, step=0.01, description='Cup Rad Major:', style=style, layout=layout)
...
ui_Tt = widgets.FloatText(value=5, step=0.01, description='Tablet Thick:', style=style, layout=layout)
...
ui_b_Ro = widgets.FloatText(value=0.381, step=0.01, description='Radius Outer:', style=style, layout=layout)
...
# =====================================================================
def compute_width_from_depth(depth, angle, ri, dc, rc_maj):
    ...
    b_quad = 2.0 * (rc_maj - d_sharp) / np.tan(half_angle)
    c_quad = d_sharp**2 - 2.0 * rc_maj * d_sharp
...
def compute_depth_from_width(target_width, angle, ri, dc, rc_maj):
    if target_width <= 0: return 0.0
    d_min, d_max = 0.0, dc * 2.0
    for _ in range(35):
        d_mid = (d_min + d_max) / 2
        w_mid = compute_width_from_depth(d_mid, angle, ri, dc, rc_maj)
...
def apply_suggested(b):
    ...
    try:
        ...
        ui_b_width.value = compute_width_from_depth(suggested_depth, 90.0, 0.061, ui_Dc.value, ui_Rc_maj.value)
    finally:
        updating = False
        draw_views()
...
def on_value_change(change):
    ...
    try:
        ...
        ui_Rc_min.value = max(0.01, ui_Rc_min.value)
        ui_Rc_maj.value = max(0.01, ui_Rc_maj.value)
        ...
        ui_b_Ro.value = max(0.0, ui_b_Ro.value)
        ...
        if sender in (ui_W, ui_L, ui_Dc, ui_Land):
            Dc = ui_Dc.value
            ui_Rc_min.value = round((w_c**2 + Dc**2) / (2 * Dc), 4)
            ui_Rc_maj.value = round((l_c**2 + Dc**2) / (2 * Dc), 4)
            if sender == ui_Dc: ui_Tt.value = round(ui_Hb.value + 2 * Dc, 4)
        elif sender == ui_Rc_min:
            R_min = max(ui_Rc_min.value, w_c + 0.01)
            ui_Rc_min.value = round(R_min, 4)
            Dc = max(0.01, R_min - np.sqrt(R_min**2 - w_c**2))
            ui_Dc.value = round(Dc, 4)
            ui_Rc_maj.value = round((l_c**2 + Dc**2) / (2 * Dc), 4)
            ui_Tt.value = round(ui_Hb.value + 2 * Dc, 4)
        elif sender == ui_Rc_maj:
            R_maj = max(ui_Rc_maj.value, l_c + 0.01)
            ui_Rc_maj.value = round(R_maj, 4)
            Dc = max(0.01, R_maj - np.sqrt(R_maj**2 - l_c**2))
            ui_Dc.value = round(Dc, 4)
            ui_Rc_min.value = round((w_c**2 + Dc**2) / (2 * Dc), 4)
            ui_Tt.value = round(ui_Hb.value + 2 * Dc, 4)
        ...
        if sender == ui_b_width:
            calc_depth = compute_depth_from_width(ui_b_width.value, ui_b_angle.value, ui_b_Ri.value, ui_Dc.value, ui_Rc_maj.value)
            if calc_depth > max_b_depth:
                ui_b_depth.value = max_b_depth
                ui_b_width.value = compute_width_from_depth(max_b_depth, ui_b_angle.value, ui_b_Ri.value, ui_Dc.value, ui_Rc_maj.value)
            else:
                ui_b_depth.value = calc_depth
        elif sender in (ui_b_depth, ui_b_angle, ui_b_Ri, ui_b_type, ui_Dc, ui_Rc_maj):
            ui_b_width.value = compute_width_from_depth(ui_b_depth.value, ui_b_angle.value, ui_b_Ri.value, ui_Dc.value, ui_Rc_maj.value)

        draw_views()
    finally:
        updating = False

for widget in (ui_W, ui_L, ui_Hb, ui_Tt, ui_Re, ui_Rs, ui_Dc, ui_Rc_min, ui_Rc_maj, ui_Land, ui_b_type, ui_b_width, ui_b_depth, ui_b_angle, ui_b_Ri, ui_b_Ro):
    widget.observe(on_value_change, names='value')

# =====================================================================
# 5. ИНСТРУМЕНТЫ РИСОВАНИЯ (Геометрия и Аннотации)
# =====================================================================
...
def get_groove_z_1d_with_ro(x_arr, depth, angle, ri, ro, dc):
    alpha = np.radians(angle / 2.0)
    d_sharp = ri / np.sin(alpha) - ri if ri > 0 else 0
    z_b = dc - depth
    x_abs = np.abs(x_arr)
   
    y_ti = ri * np.sin(alpha)
    y_o = ro / np.cos(alpha) + (depth - ro + d_sharp) * np.tan(alpha)
    y_to = y_o - ro * np.cos(alpha)
   
    z_g = np.full_like(x_arr, dc * 5.0)
   
    m_in = x_abs <= y_ti
    if np.any(m_in): z_g[m_in] = z_b + ri - np.sqrt(np.maximum(0, ri**2 - x_abs[m_in]**2))
   
    m_v = (x_abs > y_ti) & (x_abs <= y_to)
    if np.any(m_v): z_g[m_v] = z_b - d_sharp + x_abs[m_v] / np.tan(alpha)
   
    m_out = (x_abs > y_to) & (x_abs <= y_o)
    if np.any(m_out): z_g[m_out] = dc - ro + np.sqrt(np.maximum(0, ro**2 - (y_o - x_abs[m_out])**2))
...
def draw_views():
    with out:
        ...
        b_type, b_depth, b_angle, b_Ri, b_Ro = ui_b_type.value, ui_b_depth.value, ui_b_angle.value, ui_b_Ri.value, ui_b_Ro.value
        ...
        if Land > 0:
            xe_in, ys_in = l_c - (Re - Land), w_c - (Rs - Land)
            x_tan_in = xe_in + (Re - Land) * np.cos(gamma)
            mask_cup = ((xi <= x_tan_in) & ((yi - ys_in)**2 + xi**2 <= (Rs - Land)**2)) | ((xi > x_tan_in) & (yi**2 + (xi - xe_in)**2 <= (Re - Land)**2))
        else:
            mask_cup = ((xi <= x_tan) & ((yi - ys)**2 + xi**2 <= Rs**2)) | ((xi > x_tan) & (yi**2 + (xi - xe)**2 <= Re**2))

        Z = np.zeros_like(X)
        if w_c > 0 and l_c > 0:
            rho_norm = np.sqrt((X / l_c)**2 + (Y / w_c)**2)
            valid = (rho_norm <= 1) & mask_cup
        ...
        # --- 2. SIDE VIEW (Разрез профиля L - С УЧЕТОМ RADIUS OUTER) ---
        yc_up_maj = Hb/2 + Dc - Rc_maj
        x_maj_cup = np.linspace(-l_c, l_c, 200)
        z_up_maj_surf = yc_up_maj + np.sqrt(np.maximum(0, Rc_maj**2 - x_maj_cup**2))
       
        if b_type != 'None' and b_depth > 0:
            z_g_1d = get_groove_z_1d_with_ro(x_maj_cup, b_depth, b_angle, b_Ri, b_Ro, Dc)
        ...
        z_down_maj_surf = yc_up_maj + np.sqrt(np.maximum(0, Rc_maj**2 - x_maj_cup**2))
        ...
        # === ИЗМЕНЕНИЕ: Смещение стрелки Cup Radius Major вниз ===
        # Выбираем точку примерно на 1/4 длины главной оси вниз от центра
        l_target_val = -L / 4
        idx_target = np.argmin(np.abs(x_maj_cup - l_target_val))

        # Получаем координаты точки на поверхности (с учетом риски, если она есть)
        pt_surf_z = z_up_maj_surf[idx_target]
        pt_surf_l = x_maj_cup[idx_target]

        target_coords = (pt_surf_z + cx_side, pt_surf_l + cy_side)
        # Текст смещаем относительно новой точки указания
        text_coords = (pt_surf_z + cx_side + 4, pt_surf_l + cy_side - L/4)

        draw_pointer(ax, target_coords, text_coords, f"{Rc_maj:g}\nCup Radius\nMajor")
        # =========================================================


        # --- 3. FRONT VIEW (Профиль W) ---
        yc_up_min = Hb/2 + Dc - Rc_min
        x_min_cup = np.linspace(-w_c, w_c, 100)
        z_up_min = yc_up_min + np.sqrt(np.maximum(0, Rc_min**2 - x_min_cup**2))
        ...
        if b_type != 'None' and b_depth > 0:
            if b_type == 'Standard':
                z_bottom_line = (np.sqrt(np.maximum(0, Rc_min**2 - x_min_cup**2)) - (Rc_min - Dc)) - b_depth
            elif b_type == 'Cut Through':
                z_bottom_line = np.full_like(x_min_cup, Dc - b_depth)
            elif b_type == 'Decreasing':
                edge_y = w_c if w_c > 0 else W/2
                z_bottom_line = (np.sqrt(np.maximum(0, Rc_min**2 - x_min_cup**2)) - (Rc_min - Dc)) - b_depth * np.maximum(0, 1 - (x_min_cup / edge_y)**2)
        ...
        draw_pointer(ax, (cx_front, cy_front + Tt/2), (cx_front - W/4, cy_front + Tt/2 + 4), f"{Rc_min:g}\nCup Radius\nMinor")
...
col5 = widgets.VBox([ui_b_type, btn_suggest, ui_b_width, ui_b_depth, ui_b_angle, ui_b_Ri, ui_b_Ro])
