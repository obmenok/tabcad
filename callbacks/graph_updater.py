...
def _build_params(
    ...
    return {
        ...
        "W": w if w is not None else 8.0,
        "L": l if l is not None else 18.3,
        "Re": re if re is not None else 4.6,
        "Rs": rs if rs is not None else 15.0,
        "Dc": dc if dc is not None else 0.92,
        "Rc_min": rc_min if rc_min is not None else 8.8,
        "Rc_maj": rc_maj if rc_maj is not None else 39.8,
        "Land": land if land is not None else 0.08,
        "Hb": hb if hb is not None else 2.55,
        "Tt": tt if tt is not None else 4.39,
        "Bev_D": bev_d if bev_d is not None else 0.51,
        "Bev_A": bev_a if bev_a is not None else 40.0,
        "R_edge": r_edge if r_edge is not None else 6.35,
        "Blend_R": blend_r if blend_r is not None else 0.38,
        "R_maj_maj": r_maj_maj if r_maj_maj is not None else 88.9,
        "R_maj_min": r_maj_min if r_maj_min is not None else 6.35,
        "R_min_maj": r_min_maj if r_min_maj is not None else 12.7,
        "R_min_min": r_min_min if r_min_min is not None else 3.81,
        "b_type": b_type if b_type is not None else "none",
        "b_width": b_width if b_width is not None else 2.25,
        "b_depth": b_depth if b_depth is not None else 1.12,
        "b_angle": b_angle if b_angle is not None else 90.0,
        "b_Ri": b_ri if b_ri is not None else 0.06,
