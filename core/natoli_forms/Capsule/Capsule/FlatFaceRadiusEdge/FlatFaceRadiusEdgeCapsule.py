...
# Входные параметры (Капсула FFRE)
...
ui_Rc = widgets.FloatText(value=7.6841, description='Cup Radius:', disabled=True, style=style, layout=layout)
...
# =====================================================================
# 3. МАТЕМАТИКА КАПСУЛЫ (FFRE) И РИСКИ
# =====================================================================
def get_capsule_ffre_profile(rho_arr, rc, Dc, R_edge):
    """Возвращает профиль Flat Face Radius Edge по расстоянию rho от хребта капсулы"""
    z_prof = np.zeros_like(rho_arr)
    if R_edge <= Dc: return z_prof 
    
    dx_curve = np.sqrt(max(0, R_edge**2 - (R_edge - Dc)**2))
    r_flat = max(0, rc - dx_curve)
    
    mask_flat = rho_arr <= r_flat
    mask_edge = (rho_arr > r_flat) & (rho_arr <= rc)
    
    if np.any(mask_flat):
        z_prof[mask_flat] = Dc
    if np.any(mask_edge):
        z_prof[mask_edge] = (Dc - R_edge) + np.sqrt(np.maximum(0, R_edge**2 - (rho_arr[mask_edge] - r_flat)**2))
        
    return np.maximum(0, z_prof)

def compute_width_from_depth(depth, angle, ri, dc, r_edge, rc):
    ...
    z_cup = get_capsule_ffre_profile(x_test, rc, dc, r_edge)
...
def compute_depth_from_width(target_width, angle, ri, dc, r_edge, rc):
    if target_width <= 0: return 0.0
    d_min, d_max = 0.0, dc * 2.0
    for _ in range(35):
        d_mid = (d_min + d_max) / 2
        w_mid = compute_width_from_depth(d_mid, angle, ri, dc, r_edge, rc)
...
def on_value_change(change):
    ...
    try:
        ...
        if sender in (ui_W, ui_L, ui_Dc, ui_Land, ui_R_edge):
        ...
        # Синхронизация риски (ОПЕЧАТКА ИСПРАВЛЕНА!)
        ...
        elif sender in (ui_b_depth, ui_b_angle, ui_b_Ri, ui_b_type, ui_Dc, ui_W, ui_Land, ui_R_edge):
...
for widget in (ui_W, ui_L, ui_Hb, ui_Tt, ui_Dc, ui_R_edge, ui_Land, ui_b_type, ui_b_width, ui_b_depth, ui_b_angle, ui_b_Ri):
...
def draw_views():
    with out:
        ...
        # 3D Генерация свода капсулы FFRE
        Z = np.zeros_like(X)
        Z[mask_cup] = get_capsule_ffre_profile(rho[mask_cup], r_c, Dc, R_edge)
        ...
        if b_type != 'None' and b_depth > 0:
            ...
            m_in = np.abs(X) <= r_c 
            Z_centerline[m_in] = get_capsule_ffre_profile(np.abs(Y[m_in]), r_c, Dc, R_edge)
        ...
        z_up_maj_surf = get_capsule_ffre_profile(rho_maj, r_c, Dc, R_edge) + yc_up_maj
        ...
        z_down_maj_surf = get_capsule_ffre_profile(rho_maj, r_c, Dc, R_edge) + yc_up_maj
        ...
        z_up_min = yc_up_min + get_capsule_ffre_profile(rho_min, r_c, Dc, R_edge)
        ...
        # Если есть риска, на фронтальном виде мы видим дно риски (ОПЕЧАТКА Z_UP_FRONT ИСПРАВЛЕНА!)
        if b_type != 'None' and b_depth > 0:
            Z_centerline_front = get_capsule_ffre_profile(rho_min, r_c, Dc, R_edge)
        ...
        z_min_pt = get_capsule_ffre_profile(np.array([np.abs(w_target_val)]), r_c, Dc, R_edge)[0]
