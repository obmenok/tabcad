...
ui_Rc = widgets.FloatText(value=8.4515, description='Cup Radius Minor:', disabled=True, style=style, layout=layout)
...
# =====================================================================
# 3. МАТЕМАТИКА КАПСУЛЫ (CBE) И РИСКИ
# =====================================================================
def get_capsule_cbe_profile(rho_arr, rc, Dc, Db, angle_deg):
    """Возвращает профиль фаска+сфера по заданному радиусу rho от хребта"""
...
def on_value_change(change):
    ...
    try:
        ...
        r_c = W/2 - Land
        ui_Re.value = W / 2 # Синхронизация End Radius для капсулы
        ...
        # Синхронизация эквивалентного сферического радиуса (Cup Radius Minor)
        ...
        elif sender in (ui_b_depth, ui_b_angle, ui_b_Ri, ui_b_type, ui_Dc, ui_W, ui_Land, ui_Bev_D, ui_Bev_A):
...
for widget in (ui_W, ui_L, ui_Hb, ui_Tt, ui_Dc, ui_Bev_D, ui_Bev_A, ui_Land, ui_b_type, ui_b_width, ui_b_depth, ui_b_angle, ui_b_Ri):
...
def draw_views():
    with out:
        ...
        # 3D Генерация свода капсулы CBE
        Z = np.zeros_like(X)
        Z[mask_cup] = get_capsule_cbe_profile(rho[mask_cup], r_c, Dc, Db, Bev_A)
        ...
        if b_type != 'None' and b_depth > 0:
            ...
            # Риска идет вдоль Y (горизонтально), Z зависит только от Y
        ...
        if r_in > 0 and r_in < r_c:
            x_in2, y_in2 = get_capsule_contour(L - 2*Land - 2*w_b, W - 2*Land - 2*w_b)
        ...
        # Выноски размеров на Side View
        draw_ext_outside(ax, cx_side + Hb/2, cy_side + L/2, cx_side + Hb/2 + Dc, cy_side + L/2, 0, 4, f"{Dc:g}\nCup Depth")
        if Db > 0:
            draw_ext_outside(ax, cx_side + Hb/2, cy_side + L/2, cx_side + Hb/2 + Db, cy_side + L/2, 0, 2, f"{Db:g}\nBevel Depth")
        ...
        # Отрисовка угла Bevel Angle на Front View (Вверху справа)
        if Bev_A > 0 and Bev_A < 90 and Db > 0:
            ...
            # Наклонная линия фаски (вниз и вправо)
