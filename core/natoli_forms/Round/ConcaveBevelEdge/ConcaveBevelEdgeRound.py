...
ui_Rc = widgets.FloatText(value=13.5423, description='Cup Radius:', disabled=True, style=style, layout=layout)
...
# =====================================================================
# 3. МАТЕМАТИКА ФАСКИ И РИСКИ
# =====================================================================
def get_concave_bevel_profile(x_arr, span, Dc, Db, angle_deg):
    """
    Математика профиля чаши с фаской и сферическим центром.
    """
    z_prof = np.zeros_like(x_arr)
    if span <= 0 or Dc <= 0: return z_prof
    ...
    x_abs = np.abs(x_arr)
    mask_bevel = (x_abs > r_in) & (x_abs <= span)
    mask_cup = x_abs <= r_in
    ...
    if np.any(mask_cup):
        if hc > 0:
            Rc = (r_in**2 + hc**2) / (2 * hc)
            z_prof[mask_cup] = Db + np.sqrt(np.maximum(0, Rc**2 - x_abs[mask_cup]**2)) - (Rc - hc)
        else:
            z_prof[mask_cup] = Db
            
    return np.maximum(0, z_prof)

def compute_width_from_depth(depth, angle, ri, dc, db, angle_deg, span):
    ...
    z_cup = get_concave_bevel_profile(x_test, span, dc, db, angle_deg)
...
def compute_depth_from_width(target_width, angle, ri, dc, db, angle_deg, span):
    if target_width <= 0: return 0.0
    d_min, d_max = 0.0, dc * 2.0
    for _ in range(35):
        d_mid = (d_min + d_max) / 2
        w_mid = compute_width_from_depth(d_mid, angle, ri, dc, db, angle_deg, span)
...
def on_value_change(change):
    ...
    try:
        ...
        # 1. Bevel Depth не может быть больше Cup Depth
        if ui_Bev_D.value > Dc:
            ui_Bev_D.value = round(Dc, 4)
            
        # 2. Фаска не может занять всю площадку таблетки
        max_db = r_c * tan_a
        ...
        # Синхронизация эквивалентного сферического радиуса (Cup Radius)
        if sender in (ui_D, ui_Dc, ui_Land, ui_Bev_D, ui_Bev_A):
            hc = Dc - Db
            w_b = Db / tan_a
            r_in = r_c - w_b
            if hc > 0:
...
for widget in (ui_D, ui_Hb, ui_Tt, ui_Dc, ui_Bev_D, ui_Bev_A, ui_Land, ui_b_type, ui_b_width, ui_b_depth, ui_b_angle, ui_b_Ri):
...
def draw_views():
    with out:
        ...
        Z = np.zeros_like(X)
        Z[mask_cup] = get_concave_bevel_profile(rho[mask_cup], r_c, Dc, Db, Bev_A)
        ...
        if b_type != 'None' and b_depth > 0:
            ...
            m_in = np.abs(X) <= r_c
            Z_centerline[m_in] = get_concave_bevel_profile(X[m_in], r_c, Dc, Db, Bev_A)
            ...
            Y_abs = np.abs(Y)
            Z_v = z_bottom - d_sharp + Y_abs / np.tan(alpha_b)
        ...
        # Линия начала сферы (конец фаски)
        if r_in > 0 and r_in < r_c:
            x_in2, y_in2 = get_circle_contour(r_in)
            ax.plot(x_in2 + cx_top, y_in2 + cy_top, 'k-', linewidth=0.6)
        ...
        if b_type != 'None' and b_depth > 0:
            ...
            if x_ti_val > 0.005:
                idx_y = np.argmin(np.abs(y_grid - x_ti_val))
                ...
                if len(valid_indices) > 0:
                    ...
                    ax.plot([x_start + cx_top, x_end + cx_top], [x_ti_val + cy_top, x_ti_val + cy_top], 'k-', lw=0.6)
                    ax.plot([x_start + cx_top, x_end + cx_top], [-x_ti_val + cy_top, -x_ti_val + cy_top], 'k-', lw=0.6)

        draw_ext(ax, cx_top - D/2, cy_top + D/2, cx_top - D/2, cy_top - D/2, -4.5, 0, f"{D:g}\nDiameter")

        # --- 2. SIDE VIEW (Вертикальный вид слева) ---
        yc_up = Hb/2
        y_cup = np.linspace(-r_c, r_c, 400)
        z_up_surf = yc_up + get_concave_bevel_profile(y_cup, r_c, Dc, Db, Bev_A)
        ...
        z_down_surf = yc_up + get_concave_bevel_profile(y_cup, r_c, Dc, Db, Bev_A)
        ...
        if Db > 0:
            draw_ext_outside(ax, cx_side + Hb/2, cy_side + D/2, cx_side + Hb/2 + Db, cy_side + D/2, 0, 2, f"{Db:g}\nBevel Depth")
            
        draw_ext_outside(ax, cx_side - Hb/2, cy_side - D/2, cx_side + Hb/2, cy_side - D/2, 0, -4, f"{Hb:g}\nBelly Band")

        # --- 3. FRONT VIEW (Горизонтальный вид снизу) ---
        xc_up = Hb/2
        x_cup = np.linspace(-r_c, r_c, 200)
        z_up_front = xc_up + get_concave_bevel_profile(x_cup, r_c, Dc, Db, Bev_A)
        ...
        if b_type != 'None' and b_depth > 0:
            Z_centerline_front = get_concave_bevel_profile(x_cup, r_c, Dc, Db, Bev_A)
        ...
        # Отрисовка угла Bevel Angle на Front View (Справа сверху - НОВОЕ МЕСТО)
        if Bev_A > 0 and Bev_A < 90 and Db > 0:
            ...
            # Наклонная выносная линия (вниз-вправо)
        ...
        # Выноска Cup Radius (указывает в центр купола сверху)
        if Rc_disp > 0:
            draw_pointer(ax, (cx_front, cy_front + Hb/2 + Dc), (cx_front - D/4, cy_front + Hb/2 + Dc + 4), f"{Rc_disp:g}\nCup Radius")
...
col2 = widgets.VBox([ui_Bev_D, ui_Bev_A])
