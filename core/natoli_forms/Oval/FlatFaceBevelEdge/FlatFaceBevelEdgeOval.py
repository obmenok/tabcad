...
# Входные параметры (Flat Face Bevel Edge)
...
ui_Rc_min = widgets.FloatText(value=7.9038, description='Cup Radius Minor:', disabled=True, style=style, layout=layout)
...
# =====================================================================
# 3. МАТЕМАТИКА ПРОФИЛЯ FFBE И РИСКИ
# =====================================================================
def get_ffbe_profile(x_arr, span, Dc, R_blend, angle_deg):
    """Строит профиль Flat Face Bevel Edge (плоский участок -> радиус -> фаска)"""
    ...
    # Горизонтальное расстояние, которое занимает фаска и скругление
    D_inset = (Dc - R_blend) / tan_a + R_blend / sin_a
    Xc = span - D_inset
    
    x_abs = np.abs(x_arr)
    mask_flat = x_abs <= max(0, Xc)
    
    Xt = Xc + R_blend * sin_a if Xc > 0 else span - Dc / tan_a
    mask_blend = (x_abs > max(0, Xc)) & (x_abs <= Xt)
    mask_bevel = (x_abs > Xt) & (x_abs <= span)
    
    if np.any(mask_flat):
        z_prof[mask_flat] = Dc
    if np.any(mask_blend):
        z_prof[mask_blend] = (Dc - R_blend) + np.sqrt(np.maximum(0, R_blend**2 - (x_abs[mask_blend] - Xc)**2))
    if np.any(mask_bevel):
        z_prof[mask_bevel] = (span - x_abs[mask_bevel]) * tan_a
        
    return np.maximum(0, z_prof)

def compute_width_from_depth(depth, angle, ri, dc, R_blend, angle_deg, span):
    ...
    z_cup = get_ffbe_profile(x_test, span, dc, R_blend, angle_deg)
...
def compute_depth_from_width(target_width, angle, ri, dc, R_blend, angle_deg, span):
    if target_width <= 0: return 0.0
    d_min, d_max = 0.0, dc * 2.0
    for _ in range(35):
        d_mid = (d_min + d_max) / 2
        w_mid = compute_width_from_depth(d_mid, angle, ri, dc, R_blend, angle_deg, span)
...
def apply_suggested(b):
    ...
    try:
        ...
        w_c = ui_W.value / 2 - ui_Land.value
        ui_b_width.value = compute_width_from_depth(suggested_depth, 90.0, 0.061, ui_Dc.value, ui_Blend_R.value, ui_Bev_A.value, w_c)
    finally:
        updating = False
        draw_views()
...
# =====================================================================
# 4. ЛОГИКА ВАЛИДАЦИИ (С УМНЫМ ПРЕДОХРАНИТЕЛЕМ ДЛЯ FFBE)
# =====================================================================
updating = False
...
def on_value_change(change):
    ...
    try:
        ...
        sin_a = np.sin(alpha)
        cos_a = np.cos(alpha)
        span_min = min(w_c, re_c)
        
        # Защита 1: Глубина чаши не может быть больше, чем может покрыть фаска
        max_Dc = span_min * tan_a
        if Dc > max_Dc - 0.001:
            Dc = round(max_Dc - 0.001, 4)
            ui_Dc.value = Dc

        # Защита 2: Предотвращение схлопывания площадки из-за слишком большого Blend Radius
        max_rb_num = span_min * sin_a - Dc * cos_a
        ...
        # Справочный расчет Cup Radius Minor
        ...
        if sender == ui_b_width:
            ui_b_depth.value = compute_depth_from_width(ui_b_width.value, ui_b_angle.value, ui_b_Ri.value, ui_Dc.value, ui_Blend_R.value, ui_Bev_A.value, w_c)
        elif sender in (ui_b_depth, ui_b_angle, ui_b_Ri, ui_b_type, ui_Dc, ui_Blend_R, ui_Bev_A):
            ui_b_width.value = compute_width_from_depth(ui_b_depth.value, ui_b_angle.value, ui_b_Ri.value, ui_Dc.value, ui_Blend_R.value, ui_Bev_A.value, w_c)

        draw_views()
    finally:
        updating = False

for widget in (ui_W, ui_L, ui_Hb, ui_Tt, ui_Re, ui_Rs, ui_Dc, ui_Blend_R, ui_Bev_A, ui_Land, ui_b_type, ui_b_width, ui_b_depth, ui_b_angle, ui_b_Ri):
...
def draw_views():
    with out:
        ...
        Dc, R_blend, Bev_A = ui_Dc.value, ui_Blend_R.value, ui_Bev_A.value
        ...
        # Расчет сдвига D_inset для плоской площадки
        alpha = np.radians(Bev_A)
        tan_a = np.tan(alpha)
        sin_a = np.sin(alpha)
        D_inset = (Dc - R_blend) / tan_a + R_blend / sin_a

        # === ИДЕАЛЬНАЯ 3D ГЕНЕРАЦИЯ СТАБИЛИЗИРОВАННАЯ ===
        Z_L = get_ffbe_profile(X, l_c, Dc, R_blend, Bev_A)
        Z_W = get_ffbe_profile(Y, w_c, Dc, R_blend, Bev_A)
        Z = np.zeros_like(X)
        Dc_safe = max(1e-5, Dc)
        ...
        if b_type != 'None' and b_depth > 0:
            ...
            Z_centerline = get_ffbe_profile(Y, w_c, Dc, R_blend, Bev_A)
        ...
        l_flat = max(0, l_c - D_inset)
        w_flat = max(0, w_c - D_inset)
        
        # Отрисовка контура плоской части (сплошной линией)
        if l_flat > 0.05 and w_flat > 0.05:
            L_flat = L - 2*Land - 2*D_inset
            W_flat = W - 2*Land - 2*D_inset
            Re_flat = Re - Land - D_inset
            Rs_flat = Rs - Land - D_inset
            if Re_flat > 0.01 and Rs_flat > 0.01 and W_flat > 0.1 and L_flat > 0.1:
                x_flat_cont, y_flat_cont = get_oval_contour(L_flat, W_flat, Re_flat, Rs_flat)
        ...
        z_up_maj_surf = yc_up_maj + get_ffbe_profile(x_maj_cup, l_c, Dc, R_blend, Bev_A)
        ...
        z_down_maj_surf = yc_up_maj + get_ffbe_profile(x_maj_cup, l_c, Dc, R_blend, Bev_A)
        ...
        if l_flat > 0:
            draw_ext(ax, cx_side + Hb/2 + Dc, cy_side + l_flat, cx_side + Hb/2 + Dc, cy_side - l_flat, 4.5, 0, f"{2*l_flat:.3f}\nRef. Flat")

        # --- 3. FRONT VIEW (Профиль W) ---
        yc_up_min = Hb/2
        x_min_cup = np.linspace(-w_c, w_c, 200)
        z_up_min = yc_up_min + get_ffbe_profile(x_min_cup, w_c, Dc, R_blend, Bev_A)
        ...
        if b_type != 'None' and b_depth > 0:
            Z_centerline_front = get_ffbe_profile(x_min_cup, w_c, Dc, R_blend, Bev_A)
        ...
        if w_flat > 0:
            draw_ext_outside(ax, cx_front - w_flat, cy_front - Hb/2 - Dc, cx_front + w_flat, cy_front - Hb/2 - Dc, 0, -3, f"{2*w_flat:.3f}\nRef. Flat")
        ...
        # Выноска Blend Radius (Указывает на точку скругления)
        if R_blend > 0:
            w_target_val = - (w_c - D_inset + (R_blend * sin_a)/2) 
            z_min_pt = get_ffbe_profile(np.array([w_target_val]), w_c, Dc, R_blend, Bev_A)[0]
...
col3 = widgets.VBox([ui_Dc, ui_Rc_min, ui_Bev_A, ui_Blend_R])
