...
# Входные параметры (Размеры таблетки - по умолчанию Compound Cup из примера)
...
# Новые радиусы для обеих осей
ui_R_min_maj = widgets.FloatText(value=12.7, step=0.1, description='Minor Major Radius:', style=style, layout=layout)
ui_R_min_min = widgets.FloatText(value=3.81, step=0.1, description='Minor Minor Radius:', style=style, layout=layout)
ui_R_maj_maj = widgets.FloatText(value=88.9, step=0.1, description='Major Major Radius:', style=style, layout=layout)
ui_R_maj_min = widgets.FloatText(value=6.35, step=0.1, description='Major Minor Radius:', style=style, layout=layout)
...
def get_compound_profile(x_arr, R1, R2, Dc, span):
    """
    Универсальная функция расчета профиля купола с двумя сопряженными радиусами.
    Может применяться как к главной (Major), так и к малой (Minor) оси.
    """
    Z1 = Dc - R1

    if span <= 0 or R1 <= 0 or R2 <= 0: return np.zeros_like(x_arr)
    ...
    if discriminant < 0:
        return np.maximum(0, Dc * (1 - (x_arr/span)**2))
...
def compute_width_from_depth(depth, angle, ri, dc, R_maj, R_min, span):
    ...
    z_cup = get_compound_profile(x_test, R_maj, R_min, dc, span)
...
def compute_depth_from_width(target_width, angle, ri, dc, R_maj, R_min, span):
    if target_width <= 0: return 0.0
    d_min, d_max = 0.0, dc * 2.0
    for _ in range(35):
        d_mid = (d_min + d_max) / 2
        w_mid = compute_width_from_depth(d_mid, angle, ri, dc, R_maj, R_min, span)
...
def on_value_change(change):
    ...
    try:
        ...
        ui_R_min_maj.value = max(0.01, ui_R_min_maj.value)
        ui_R_min_min.value = max(0.01, ui_R_min_min.value)
...
for widget in (ui_W, ui_L, ui_Hb, ui_Tt, ui_Re, ui_Rs, ui_Dc, ui_R_min_maj, ui_R_min_min, ui_R_maj_maj, ui_R_maj_min, ui_Land, ui_b_type, ui_b_width, ui_b_depth, ui_b_angle, ui_b_Ri):
...
def draw_views():
    with out:
        ...
        Dc = ui_Dc.value
        R_min_maj, R_min_min = ui_R_min_maj.value, ui_R_min_min.value
        ...
        # === COMPOUND CUP 3D ГЕНЕРАЦИЯ ===
        Z_L = get_compound_profile(X, R_maj_maj, R_maj_min, Dc, l_c)
        Z_W = get_compound_profile(Y, R_min_maj, R_min_min, Dc, w_c)
        ...
        if b_type != 'None' and b_depth > 0:
            ...
            Z_centerline = get_compound_profile(Y, R_min_maj, R_min_min, Dc, w_c)
        ...
        z_up_maj_surf = yc_up_maj + get_compound_profile(x_maj_cup, R_maj_maj, R_maj_min, Dc, l_c)
        ...
        z_down_maj_surf = yc_up_maj + get_compound_profile(x_maj_cup, R_maj_maj, R_maj_min, Dc, l_c)
        ...
        z_up_min = yc_up_min + get_compound_profile(x_min_cup, R_min_maj, R_min_min, Dc, w_c)
        ...
        if b_type != 'None' and b_depth > 0:
            Z_centerline_front = get_compound_profile(x_min_cup, R_min_maj, R_min_min, Dc, w_c)

            if b_type == 'Standard':
                z_bottom_line = Z_centerline_front - b_depth
            elif b_type == 'Cut Through':
        ...
        # === НОВЫЕ СТРЕЛКИ ДЛЯ FRONT VIEW (ИСПРАВЛЕНА ПОЗИЦИЯ И НАКЛОН) ===
        # Minor Major Radius
        x_min_maj_pt = w_c * 0.1
        z_min_maj_pt = get_compound_profile(np.array([x_min_maj_pt]), R_min_maj, R_min_min, Dc, w_c)[0]
        ...
        # Minor Minor Radius (Кончик на месте, линия наклонена влево)
        x_min_min_pt = w_c * 0.80
        z_min_min_pt = get_compound_profile(np.array([x_min_min_pt]), R_min_maj, R_min_min, Dc, w_c)[0]
        targ_front_min = (cx_front - x_min_min_pt, cy_front + Hb/2 + z_min_min_pt)
        # Изменяем координату текста по X, сдвигая ее влево (-2.5 вместо -0.5)
...
col3 = widgets.VBox([ui_Dc, ui_R_min_maj, ui_R_min_min])
col4 = widgets.VBox([ui_R_maj_maj, ui_R_maj_min, ui_Land])
